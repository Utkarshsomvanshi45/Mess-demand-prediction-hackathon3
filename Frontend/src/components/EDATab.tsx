...
import { Database, BarChart3, Activity, Calendar } from 'lucide-react';

const DEMAND_DIST = [
  { name: 'High', value: 340, color: '#ef4444' },
  { name: 'Medium', value: 420, color: '#f59e0b' },
  { name: 'Low', value: 260, color: '#10b981' },
];

const DEMAND_BY_MEAL = [
  { meal: 'Breakfast', High: 100, Medium: 140, Low: 100 },
  { meal: 'Lunch', High: 150, Medium: 160, Low: 90 },
  { meal: 'Dinner', High: 90, Medium: 120, Low: 70 },
];

const OCCUPANCY_BOX = [
  { level: 'High', min: 72, q1: 78, median: 85, q3: 92, max: 98 },
  { level: 'Medium', min: 50, q1: 60, median: 70, q3: 78, max: 88 },
  { level: 'Low', min: 30, q1: 38, median: 45, q3: 52, max: 62 },
];

const PHASE_DEMAND = [
  { phase: 'Regular', High: 240, Medium: 300, Low: 160 },
  { phase: 'Exams', High: 60, Medium: 80, Low: 60 },
  { phase: 'Holidays', High: 40, Medium: 40, Low: 40 },
];

const DAY_DEMAND = [
  { day: 'Mon', High: 52, Medium: 64, Low: 38 },
  { day: 'Tue', High: 48, Medium: 60, Low: 36 },
  { day: 'Wed', High: 50, Medium: 62, Low: 40 },
  { day: 'Thu', High: 46, Medium: 58, Low: 34 },
  { day: 'Fri', High: 54, Medium: 66, Low: 42 },
  { day: 'Sat', High: 44, Medium: 56, Low: 38 },
  { day: 'Sun', High: 46, Medium: 54, Low: 32 },
];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: Activity, label: 'Avg Occupancy', value: '74.2%' },
          { icon: BarChart3, label: 'High Demand Count', value: '340' },
          { icon: Calendar, label: 'Weekend Records', value: '292' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-surface p-6">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie data={DEMAND_DIST} cx="50%" cy="50%" innerRadius={55} outerRadius={85} dataKey="value" animationDuration={1500} paddingAngle={3}>
              ...
              return (
                <div key={item.level} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 relative h-8">
                    {/* whisker line */}
                    <div className="absolute top-1/2 -translate-y-1/2 h-px bg-muted-foreground/40" style={{ left: `${leftOffset}%`, width: `${rangeWidth}%` }} />
                    {/* IQR box */}
                    <div className="absolute top-1 h-6 rounded" style={{ left: `${leftOffset + (iqrLeft / 100) * rangeWidth}%`, width: `${(iqrWidth / 100) * rangeWidth}%`, backgroundColor: `${color}22`, border: `1px solid ${color}` }} />
                    {/* median */}
                    <div className="absolute top-0.5 w-0.5 h-7" style={{ left: `${leftOffset + (medianPos / 100) * rangeWidth}%`, backgroundColor: color }} />
                    {/* labels */}
